...
확률은 통계학이든, 머신러닝이든 데이터를 측정하고, 인과관계를 추론하고, 미래를 예측하는 데 있어 현재까지는 제일 유용한 수학적 개념입니다 [1]. 물론 이런 접근이 100% 성공을 보장하는 것은 아닙니다. 그러나 **불확실성의 측도(measure of uncertainty)** 라는 관점에서 확률은 우리가 기하학에서 배우는 길이, 넓이라는 단어처럼 일상적인 용어가 되었고 현대 과학에서는 이미 떼놓을 수 없는 도구가 되었습니다. 현실 문제에서는 임상 실험이나 주식 투자, 기업 의사결정 같은 문제에서 가변적이고 다루기 까다로운 조건들을 변수로 고려해야 할 때 확률을 사용해야 합니다. 
...
여기서 \\( P \\) 는 고정된 비용이므로 결정적(deterministic) 변수 입니다 (낚시꾼 사장님은 마음이 착해서 시급을 최저임금보다 높게 준다고 가정합시다). 반면 2, 3번의 \\( M(i,t), N(t)\\) 는 **확률적인(stochastic)** 변수로 매시간마다 바뀌게 됩니다. 만약 직원을 전체 테이블 숫자만큼 채용하게 되면 테이블이 모두 꽉차는 \\( (N = 10) \\) 피크 시간대에는 괜찮을지 몰라도 그렇지 않은 시간대는 \\( (N < 10) \\) 손해를 볼 수 있습니다. 그러므로 낚시꾼 사장님은 이런 리스크를 고려해서 매출과 비용을 계산하여 적정 직원수를 채용해야 합니다. 낚시꾼 사장님이 하루에 \\( T \\) 시간동안 이 식당을 운영한다고 했을 때, 직원 \\( X \\) 명을 뽑았을 때 발생하는 매출 \\( I \\) 과 비용 \\( C \\) 을 계산해보면 아래와 같습니다:

$$
I := \sum_{t=1}^{T}\sum_{i=1}^{X \wedge N(t)} M(i,t), \quad C := \sum_{t=1}^{T}PX = PXT
$$

기호 \\( X \wedge N(t) \\) 는 최소값 \\( \min\\{X, N(t)\\} \\) 을 말합니다. 사장님이 이 식당을 손해 없이 운영하려면 매출이 비용보다 많은 \\( C < I \\) 상태를 유지하는 \\( X \\) 를 찾아야합니다. 그러나 현실적으로 \\( I \\) 가 \\( C \\) 보다 항상 많기를 바라는 건 무리입니다. 왜냐하면 손님 수 \\( N(t) \\) 와 수입 \\( M(i,t) \\) 는 확률변수라서 최악의 경우일 때는 비용이 매출보다 큰 날도 발생할 수 있기 때문입니다. \\( X \\) 를 극단적으로 줄여서 운영하면 가능할수도 있지만 사장님은 부자가 되고 싶기 때문에 이윤 극대화를 위해선 리스크를 좀 감당하려고 합니다. 그래서 경영 목표를 다음과 같이 바꿨습니다.

$$
\mathbb{P}(C > I)<\alpha \quad\cdots\quad(\star)
$$

여기서 \\( \alpha \\) 는 매우 작은 값으로 (0.01 정도?) 낚시꾼 사장님이 감당할 리스크의 크기로 해석할 수 있습니다. 즉 \\( (\star) \\) 을 이용하면 비용이 매출을 초과할 확률이 \\( \alpha \\) 미만이 되도록 \\( X \\) 를 찾는 문제가 됩니다. 고로 낚시꾼 사장님은 통계적으로 손해를 보지 않는 범위 안에서 식당 운영이 가능해집니다.

그렇다면 남은 질문은, \\( (\star) \\) 좌변의 확률값은 어떻게 계산해야 할지입니다. 위의 예제를 다시 들여다보시면 결국 확률변수 \\( M(i,t), N(t) \\) 에 따른 매출 \\( I \\) 의 **확률분포(distribution)** 를 알 수 있어야 \\( (\star) \\) 계산이 가능해집니다. 결론적으로 통계적 모델은 **적절한(feasible)** 가정 위에서 확률분포를 **추론(inference)** 해내는 것이 목표입니다.
