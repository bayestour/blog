...
Bayesian 방법론은 흔히들 [Bayes 정리 (Bayes' theorem)](https://en.wikipedia.org/wiki/Bayes%27_theorem) 에 근간을 둔다고 합니다. 틀린 말은 아닙니다만, Bayes 정리를 사용한다고 모델이 Bayesian 이 된다는 뜻이 아닙니다. Bayesian 방법론의 기본적 원리는 모든 **불확실성을 확률로 다루는 것** 입니다.
