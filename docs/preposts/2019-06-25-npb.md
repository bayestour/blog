...
여기서 \\( P \\) 는 고정된 비용이므로 결정적(deterministic) 변수 입니다 (낚시꾼 사장님은 마음이 착해서 시급을 최저임금보다 높게 준다고 가정합시다). 반면 2, 3번의 \\( M(i,t), N(t)\\) 는 **확률적인(stochastic)** 변수로 매시간마다 바뀌게 됩니다. 만약 직원을 전체 테이블 숫자만큼 채용하게 되면 테이블이 모두 꽉차는 \\( (N = 10) \\) 피크 시간대에는 괜찮을지 몰라도 그렇지 않은 시간대는 \\( (N < 10) \\) 손해를 볼 수 있습니다. 그러므로 낚시꾼 사장님은 이런 리스크를 고려해서 매출과 비용을 계산하여 적정 직원수를 채용해야 합니다. 낚시꾼 사장님이 하루에 \\( T \\) 시간동안 이 식당을 운영한다고 했을 때, 직원 \\( X \\) 명을 뽑았을 때 발생하는 매출 \\( I \\) 과 비용 \\( C \\) 은 아래와 같습니다:

$$
I := \sum_{t=1}^{T}\sum_{i=1}^{X \wedge N(t)} M(i,t), \quad C := \sum_{t=1}^{T}PX = PXT
$$

기호 \\( X \wedge N(t) \\) 는 최소값 \\( \min\\{X, N(t)\\} \\) 을 말합니다. 사장님이 이 식당을 손해 없이 운영하려면 매출이 비용보다 많은 \\( C < I \\) 상태를 유지해야 합니다. 이를 정리하면 아래와 같습니다:

$$
Y(X):=\frac{1}{T}\sum_{t=1}^{T}\sum_{i=1}^{X\wedge N(t)}\frac{M(i,t)}{P} < X \quad\cdots\quad(\star)
$$

그러나 현실적으로 부등식 \\( (\star) \\) 이 항상 만족되길 바라는 건 무리입니다. 왜냐하면 두 확률변수 \\( N(t), M(i,t) \\) 가 최악의 경우로 나왔을 때도 \\( (\star) \\) 을 만족시키려면 직원을 아예 뽑지 말아야 하는 결론이 나올 수 있기 때문입니다. 그러므로 실제로는 \\( \star \\) 을 변형한 확률적인 부등식을 도입하게 됩니다.

$$
\mathbb{P}\left(Y(X) > X \right) = \mathbb{P}(C > I)<\alpha \quad\cdots\quad(\star\star)
$$

여기서 \\( \alpha \\) 는 매우 작은 값으로 (0.01 정도?) 설정합니다. 부등식 \\( (\star\star) \\) 을 이용하면 비용이 매출을 초과할 확률이 \\( \alpha \\) 미만이 되도록 \\( X \\) 를 찾는 문제가 됩니다. 고로 낚시꾼 사장님은 \\( (\star\star) \\) 을 도입해서 통계적으로 손해를 보지 않는 범위 안에서 식당 운영이 가능해집니다. 그렇다면 남은 질문은 \\( X \\) 마다 저 확률값을 어떻게 계산할 수 있을까요? 결론적으로 \\( Y(X) \\) 의 **확률분포(distribution)** 를 알 수 있어야 위의 모델링이 가능해집니다.
