...
그렇다면 남은 질문은 \\( (\star) \\) 좌변의 확률값은 어떻게 계산할 수 있을까요? 위의 예제를 다시 들여다보시면 \\( M(i,t), N(t) \\) 에 따른 \\( I \\) 의 **확률분포(distribution)** 를 알 수 있어야 \\( (\star) \\) 계산이 가능해집니다. 결론적으로 통계적 모델은 **적절한(feasible)** 가정 위에서 확률분포를 **추론(inference)** 해내는 것이 목표입니다.
