...
$$
\mathbb{KL}(p \|\| q) = \mathbb{E}_p\left[\log(\frac{p(x)}{q(x)})\right] \quad\cdots\quad (4)
$$

여기서 \\(\mathbb{E}_p\\)는 확률분포 p에 대한 기댓값이라는 뜻입니다. 그런데 (4) 의 우변은 다음과 같이 다시 쓸 수 있습니다.

$$
\mathbb{E}_{p}\left[\log(\frac{p(x)}{q(x)})\right] = -\mathbb{E}_{p}\left[ -\log(\frac{p(x)}{q(x)}) \right] = -\mathbb{E}_{p}\left[ \log(\frac{q(x)}{p(x)}) \right] \quad\cdots\quad (5)
$$
...
$$
-\mathbb{E}_{p}\left[ \log(\frac{q(x)}{p(x)}) \right] \ge 0, \quad\mathbb{KL}(p \|\| q) = \mathbb{E}_{p}\left[ \log(\frac{p(x)}{q(x)}) \right] \ge 0,  \quad\cdots\quad (7)
$$
